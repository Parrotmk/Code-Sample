 ...
 */
class PMinifyJsCss_Minify_Model_Observer
{
	...
	public function getBlockMinifyStatus()
	{
		...
		if ($this->blockMinifyFlag === null) {
			...
			if (Mage::getStoreConfigFlag('PMinifyJsCss_minify/minify_html/enabled')
				&& Mage::getStoreConfigFlag('PMinifyJsCss_minify/minify_html/compatibility')
			) {
	...
	public function getMaxMinifyStatus()
	{
		...
		if ($this->maxMinifyFlag === null) {
			...
			$this->maxMinifyFlag = Mage::getStoreConfigFlag('PMinifyJsCss_minify/minify_html/max_minification');
		...
		if (Mage::getStoreConfigFlag('PMinifyJsCss_minify/minify_html/enabled')
			...
			if (stripos($html, '<!DOCTYPE html') !== false) {
				...
				if ($type) {
					...
					if (Mage::getStoreConfigFlag('PMinifyJsCss_minify/minify_html/max_minification')) {
